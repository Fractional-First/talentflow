-- Drop and recreate the get_public_profile function to include profile_version
...
BEGIN
  ...
  SELECT p.profile_slug, p.profile_data, p.profile_version, p.first_name, p.last_name, p.ispublished
  INTO profile_record
...
-- Drop and recreate the get_public_profile_by_id function to include profile_version
...
SET search_path = public, pg_temp
AS $func$
  SELECT p.profile_slug, p.profile_data, p.profile_version, p.first_name, p.last_name
  FROM public.profiles AS p
