-- Admin Search: add location_filter and role_filter text[] params.
-- Also add get_candidate_facets() for populating filter options.

-- Drop current signature (with text[] for status/type but no location/role)
