-- Fix get_candidate_details to include guest profiles.
-- Previously used INNER JOIN to auth.users, which excluded guests (no auth record).
-- Now uses LEFT JOIN and falls back to profiles.first_name/last_name for name resolution.
