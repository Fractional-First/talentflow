-- Update handle_new_user() trigger to support dual-portal signup
--
-- When a user signs up via the client portal (signup_source = 'client_portal'),
-- insert into client_profiles instead of profiles.
-- All other signups (candidate portal) continue to insert into profiles as before.
...
DECLARE
  ...
  signup_source text;
BEGIN
  -- Check which portal this signup came from
  signup_source := NEW.raw_user_meta_data ->> 'signup_source';

  IF signup_source = 'client_portal' THEN
    -- Client portal signup: insert into client_profiles
    BEGIN
      INSERT INTO public.client_profiles (user_id, is_onboarded)
      VALUES (NEW.id, false);
    EXCEPTION
      WHEN unique_violation THEN
        -- Row already exists; do not overwrite
        NULL;
    END;
